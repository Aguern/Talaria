...
# Modern Argon2id password hasher (OWASP recommendation 2025)
ph = PasswordHasher()
...
async def get_current_active_user(token: str = Depends(oauth2_scheme), db: AsyncSession = Depends(get_db)):
    ...
    # Return CurrentUser object with both user and tenant info
    return schemas.CurrentUser(user=user, tenant=user.tenant)